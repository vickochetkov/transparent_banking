...
while true
  ...
  puts "[1.2] Search bank or product by name"
  puts "[2] See one bank, product or category"
  ...
  if input_optn == "1"
    ...
    puts "Choose what to see:"
    puts "[1] Search bank by name"
    puts "[2] Search bank product by name"    
    ...
    if user_ch == "1"
      ...
      search_t = gets.chomp
      puts "Here the matching bank(s): "
      response = Unirest.get("http://localhost:3000/banks?search=#{search_t}")
      bank = response.body
      puts JSON.pretty_generate(bank)

    elsif user_ch == "2"
      puts "Enter search string: "
      search_str = gets.chomp
      puts "Here the matching bank product(s): "
      response = Unirest.get("http://localhost:3000/products?search=#{search_str}")
      ...
      puts "Choose what to see:"
      puts "[1] See one bank"
      puts "[2] See one bank product"    
      puts "[3] See one category"    

      user_ch = gets.chomp

      if user_ch == "1"
        puts "Enter bank ID to show:"
        input_id = gets.chomp
        response = Unirest.get("http://localhost:3000/banks/#{input_id}")
        bank = response.body
        puts JSON.pretty_generate(bank)

      elsif user_ch == "2"
        puts "Enter bank product ID to show:"
        input_id = gets.chomp
        ...
        puts "Enter category ID to show:"
        input_id = gets.chomp
